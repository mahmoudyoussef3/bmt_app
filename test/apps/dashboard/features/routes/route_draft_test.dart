...
RouteDraft _readyDraft() {
  ...
  draft = draft.replaceStop(
    1,
    draft.destination.withPlace(_place('Smart Village, GZ, Egypt', 30.07, 31.02)),
  );
...
void main() {
  ...
  group('RouteDraft', () {
    ...
    test('picking a place fills title, area and coordinates together', () {
      final stop = RouteDraft.blank(suggestedCode: 'RT-01').origin
          .withPlace(_place('El-Marg, QH, Egypt', 30.15, 31.33));
    ...
    test('a stop without a location blocks the save and names itself', () {
      ...
      expect(withName.isReady, isFalse);
      final issue = withName.issues.first;
      expect(issue.message, contains('القناطر'));
    ...
    test('toRoute numbers stations, shortens cities and maps boarding', () {
      ...
      expect(route.stations.map((station) => station.order).toList(), [1, 2, 3]);
