-- migration_20260706110000_fix_booking_status_vocabulary.sql
--
-- Reconciles the booking-flow RPCs with the decoupled status vocabulary that
-- migration 20260704220500_separate_booking_payment_status.sql introduced.
--
-- That later migration added:
--   CHECK (status in ('draft','reserved','confirmed','boarded','completed','cancelled'))
-- but the RPCs from 20260704000000_production_booking_flow_hardening.sql still
-- wrote the OLD lifecycle values ('pending','approved','under_review') into
-- operation_bookings.status, which the constraint now rejects. Booking via
-- InstaPay/bank/wallet therefore failed with:
--   new row for relation "operation_bookings" violates check constraint
--   "valid_booking_status" (23514)
--
-- Status mapping applied here:
--   awaiting payment review  ->  status = 'reserved'   (was 'pending')
--   payment approved         ->  status = 'confirmed'  (was 'approved')
--   payment_status label     ->  'underReview'         (was 'under_review')
--
-- Only status/payment_status literals changed; all other logic is unchanged.
...
begin
  ...
  insert into public.operation_bookings (
    ...
    p_dropoff_point_name, 'reserved', v_booking_number, p_receipt_url, 'client',
