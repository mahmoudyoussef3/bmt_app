...
class TripProgressSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (state is TrackingLoading) {
      return _hint(context, 'Loading live position…');
    }
    if (state is TrackingError) {
      return _hint(context, 'Live position unavailable right now.');
    }
    final progress = (state as TrackingLoaded).progress;
    if (progress == null || !progress.hasVehicleFix) {
      return _hint(context, 'Waiting for the vehicle\'s live position…');
    }
