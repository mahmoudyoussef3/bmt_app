...
class SupabaseTripsDatasource implements TripsDatasource {
  ...
  TripModel _mapBookingToTripModel(Map<String, dynamic> data) {
    ...
    return TripModel(
      ...
      driverName: driverObj?['full_name']?.toString() ?? 'Unknown Driver',
      driverInitials: (driverObj?['full_name']?.toString() ?? 'UD').substring(0, 2).toUpperCase(),
      driverRating: 4.8,
      vehicleName: vehicleObj?['brand']?.toString() ?? 'Vehicle',
      vehicleType: vehicleObj?['vehicle_type']?.toString() ?? 'Standard Shuttle',
      vehicleId: vehicleObj?['id']?.toString() ?? '',
      seats: [data['seat']?.toString() ?? '1'],
