...
class SupabasePackagesDatasource implements PackagesDatasource {
  ...
  Future<PackageSelectionDataModel> getSelectionData() async {
    ...
    final routesFuture = _supabase.from('routes').select('pickup, destination').eq('status', 'active');
    ...
    final pickups = routesData.map((e) => e['pickup']?.toString() ?? '').toSet().toList();
    final destinations = routesData.map((e) => e['destination']?.toString() ?? '').toSet().toList();
    final routeNames = routesData.map((e) => '${e['pickup']} - ${e['destination']}').toSet().toList();
