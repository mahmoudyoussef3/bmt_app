...
class _CaptainRequestsScreenState extends State<CaptainRequestsScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CaptainRequestsCubit, CaptainRequestsState>(
      listenWhen: (p, c) =>
          c is CaptainRequestsLoaded && c.actionError != null,
      ...
      builder: (context, state) {
        return switch (state) {
          ...
          CaptainRequestsLoaded(:final requests, :final pendingCount) =>
            _list(context, requests, pendingCount),
        };
      },
    );
  }
        ...
        Padding(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text(
                  'طلبات انضمام الكباتن — $pendingCount قيد المراجعة',
