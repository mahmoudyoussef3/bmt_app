...
class LiveOpsCubit extends Cubit<LiveOpsState> {
  ...
  LiveOpsCubit({required this.tripRepo, required this.driverRepo, required this.eventBus}) : super(LiveOpsLoading()) {
    _subscribe();
  }
  ...
  void _emit() => emit(LiveOpsLoaded(trips: List.from(_trips), drivers: List.from(_drivers), events: List.from(_events)));

  Future<void> cancelTrip(String tripId) async {
    await tripRepo.cancelTrip(tripId);
    eventBus.emit(TripEvent(id: 'EVT-${DateTime.now().microsecondsSinceEpoch}', tripId: tripId, type: EventType.incidentReported, severity: EventSeverity.warning, message: 'Trip $tripId cancelled by operator'));
  }

  Future<void> completeTrip(String tripId) async {
    await tripRepo.completeTrip(tripId);
    eventBus.emit(TripEvent(id: 'EVT-${DateTime.now().microsecondsSinceEpoch}', tripId: tripId, type: EventType.tripStarted, severity: EventSeverity.info, message: 'Trip $tripId marked completed'));
  }

  Future<void> reassignDriver(String tripId, String driverId) async {
    await tripRepo.reassignDriver(tripId, driverId);
    eventBus.emit(TripEvent(id: 'EVT-${DateTime.now().microsecondsSinceEpoch}', tripId: tripId, type: EventType.incidentReported, severity: EventSeverity.info, message: 'Trip $tripId reassigned to $driverId'));
  }
