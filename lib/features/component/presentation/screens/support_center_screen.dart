...
class SupportTicket {
  ...
  final List<Map<String, String>> conversation; // [{sender: 'user/agent', text: '...', time: '...'}]
...
class _SupportCenterScreenState extends State<SupportCenterScreen> {
  ...
  // List of mock user tickets
  late List<SupportTicket> _myTickets;
  ...
  void initState() {
    ...
    _myTickets = [
      SupportTicket(
        ...
        description: 'I arrived at the stop at 8:28 AM but the shuttle was already gone. I had to take a taxi.',
        priority: 'High',
        status: TicketStatus.inProgress,
        dateCreated: 'Jun 2, 2026',
        attachedImages: const ['screenshot_828am_map.png'],
        conversation: [
          {'sender': 'user', 'text': 'I missed my shuttle because the captain left before scheduled departure time.', 'time': '10:15 AM'},
          {'sender': 'agent', 'text': 'Apologies for the inconvenience, Ahmed. We are checking the shuttle logs and will update you shortly.', 'time': '11:04 AM'},
        ],
      ),
      SupportTicket(
        id: '#TK-8139',
        ...
        description: 'The screen turned grey when I pressed seat 14. I had to restart the app.',
        priority: 'Medium',
        status: TicketStatus.resolved,
        dateCreated: 'May 28, 2026',
        conversation: [
          {'sender': 'user', 'text': 'Grey screen error during checkout.', 'time': '02:30 PM'},
          {'sender': 'agent', 'text': 'This is fixed in the latest client version. Please update your application.', 'time': '03:12 PM'},
          {'sender': 'user', 'text': 'Working fine now, thanks.', 'time': '04:00 PM'},
        ],
      ),
    ];
  }
  ...
  void _submitNewTicket() {
    ...
    final newTicket = SupportTicket(
      id: '#TK-${math.Random().nextInt(9000) + 1000}',
      ...
      attachedImages: _ticketImageAttached ? ['uploaded_issue_photo.png'] : [],
      conversation: [
        {'sender': 'user', 'text': desc, 'time': 'Just now'},
      ],
    );
  ...
  void _submitRefundRequest() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Refund request submitted successfully! We will review your request in 2-3 business days.'),
  ...
  void _sendChatMessage() {
    ...
    final updatedConversation = List<Map<String, String>>.from(_activeTicket!.conversation)
      ..add({'sender': 'user', 'text': reply, 'time': 'Just now'});

    final updatedTicket = SupportTicket(
      id: _activeTicket!.id,
      category: _activeTicket!.category,
      title: _activeTicket!.title,
      description: _activeTicket!.description,
      priority: _activeTicket!.priority,
      status: _activeTicket!.status,
      dateCreated: _activeTicket!.dateCreated,
      attachedImages: _activeTicket!.attachedImages,
    ...
    setState(() {
      ...
      // Also update in list
      final idx = _myTickets.indexWhere((t) => t.id == updatedTicket.id);
      if (idx != -1) {
        _myTickets[idx] = updatedTicket;
      }
    ...
    Timer(const Duration(milliseconds: 1500), () {
      if (!mounted || _activeTicket?.id != updatedTicket.id) return;
      final agentConversation = List<Map<String, String>>.from(_activeTicket!.conversation)
        ..add({'sender': 'agent', 'text': 'Thank you for updating the ticket. An agent has been notified and is checking this.', 'time': '1 min ago'});

      final finalTicket = SupportTicket(
        id: _activeTicket!.id,
        category: _activeTicket!.category,
        title: _activeTicket!.title,
        description: _activeTicket!.description,
      ...
      setState(() {
        _activeTicket = finalTicket;
        final idx = _myTickets.indexWhere((t) => t.id == finalTicket.id);
        if (idx != -1) {
          _myTickets[idx] = finalTicket;
        }
      });
    });
  }
  ...
  Widget _buildHomeView(ColorScheme scheme) {
    return ListView(
      ...
      children: [
        ...
        const Text('Browse Help Categories', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 0.5)),
        ...
        Row(
          ...
          children: [
            const Text('Your Active Tickets', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 0.5)),
            TextButton.icon(
              onPressed: () => setState(() => _currentView = 2),
              icon: Icon(Icons.add, size: 14, color: scheme.primary),
              label: Text('New Ticket', style: TextStyle(fontSize: 12, color: scheme.primary, fontWeight: FontWeight.bold)),
            ),
          ],
        ),
  ...
  Widget _buildRefundPromoBanner(ColorScheme scheme) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [scheme.error.withAlpha(45), scheme.tertiary.withAlpha(25)]),
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: scheme.error.withAlpha(30), shape: BoxShape.circle),
            child: Icon(Icons.currency_exchange_rounded, color: scheme.error, size: 20),
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(fontSize: 11, color: scheme.onSurface.withAlpha(180)),
                ),
              ],
            ),
          ),
          ElevatedButton(
            ...
            child: const Text('Refund', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
          )
        ],
      ),
    );
  ...
  Widget _buildTicketsList(ColorScheme scheme) {
    if (_myTickets.isEmpty) {
      return const AppCard(
        ...
        child: Center(
          child: Text('No active tickets found. Create one above.', style: TextStyle(fontSize: 12, color: Colors.grey)),
        ),
      );
    }
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(ticket.id, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                          ...
                          Text(ticket.dateCreated, style: const TextStyle(fontSize: 11, color: Colors.grey)),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Text(ticket.title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                      ...
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              const Icon(Icons.forum_outlined, size: 14, color: Colors.grey),
                              const SizedBox(width: 6),
                              Text('${ticket.conversation.length} responses', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                            ],
                          ),
                          Row(
                            children: [
                              Text('Track Resolution', style: TextStyle(fontSize: 11, color: scheme.primary, fontWeight: FontWeight.bold)),
      ...
      children: [
        Expanded(
          child: ListView(
            ...
            children: [
              // Ticket Title
              const Text('Ticket Title', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              TextField(
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              onChanged: (val) => setState(() => _selectedCreateCategory = val!),
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              onChanged: (val) => setState(() => _selectedCreatePriority = val!),
                              dropdownColor: scheme.surface,
                              isExpanded: true,
                            ),
                          ),
              ...
              // Description Box
              const Text('Detailed Description', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              TextField(
                controller: _ticketDescController,
                maxLines: 4,
                decoration: InputDecoration(
                  hintText: 'Provide details about what happened. Include shuttle plate number if applicable.',
                  fillColor: scheme.surface,
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(16)),
                ),
              ),
              ...
              // Upload File Section
              const Text('Attachments (Optional)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
        ...
        _buildStickyCTAButton('Submit Support Ticket', _submitNewTicket, scheme),
      ],
    );
  }
        ...
        child: const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 6),
              Text('Attach Screenshot / Photo', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
              Text('Select from library', style: TextStyle(fontSize: 10, color: Colors.grey)),
            ],
          ),
        ),
      ),
        ...
        Expanded(
          child: ListView(
            ...
            children: [
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 6),
                        Text('Original Description', style: TextStyle(fontSize: 11, color: scheme.onSurface.withAlpha(150), fontWeight: FontWeight.bold)),
                      ],
                    ),
                    const SizedBox(height: 8),
                    ...
                    if (ticket.attachedImages.isNotEmpty) ...[
                      ...
                      Row(
                        children: [
                          Icon(Icons.attachment_rounded, size: 14, color: scheme.primary),
                          const SizedBox(width: 6),
                          Text(ticket.attachedImages.first, style: TextStyle(fontSize: 11, color: scheme.primary, decoration: TextDecoration.underline)),
                        ],
                      )
                    ]
                  ],
              ...
              // Chat Thread bubbles
              const Center(child: Text('Support Thread', style: TextStyle(fontSize: 10, color: Colors.grey, fontWeight: FontWeight.bold))),
              const SizedBox(height: 12),
              ...ticket.conversation.map((msg) {
                final isUser = msg['sender'] == 'user';
                return _buildChatBubble(msg['text']!, msg['time']!, isUser, scheme);
              }),
            ],
          ),
        ),
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  Text(ticket.id, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                  const SizedBox(width: 8),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                    decoration: BoxDecoration(
                    ...
                    child: Text(
                      ticket.statusLabel,
                      style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: ticket.statusColor(scheme)),
                    ),
                  ),
                ],
              ),
              TextButton(
                ...
                child: Row(
                  children: [
                    Text('Track Timeline', style: TextStyle(fontSize: 12, color: scheme.primary, fontWeight: FontWeight.bold)),
                    const SizedBox(width: 4),
          ...
          Align(
            ...
            child: Text(ticket.title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  Widget _buildChatBubble(String text, String time, bool isUser, ColorScheme scheme) {
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isUser ? null : Border.all(color: scheme.outline.withAlpha(50)),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
          ...
          child: Row(
            ...
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Resolution Status for ${ticket.id}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(height: 4),
                  Text(ticket.title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                ],
              ),
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(color: stepColor, width: isCurrent ? 5 : 2),
                          ),
                          child: isDone
                        ...
                        if (idx < steps.length - 1)
                          Expanded(
                            child: Container(
                              width: 2,
                              color: isDone ? Colors.green : scheme.outline.withAlpha(80),
                            ),
                          ),
                      ],
                    ),
  ...
  Widget _buildRefundRequestView(ColorScheme scheme) {
    return Column(
      ...
      children: [
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              const Text('Select Target Booking', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 14),
                decoration: BoxDecoration(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    onChanged: (val) => setState(() => _selectedRefundBooking = val!),
                    dropdownColor: scheme.surface,
                    isExpanded: true,
                  ),
                ),
              ...
              const Text('Reason for Refund', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 14),
                decoration: BoxDecoration(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    onChanged: (val) => setState(() => _selectedRefundReason = val!),
                    dropdownColor: scheme.surface,
                    isExpanded: true,
                  ),
                ),
              ...
              const Text('Refund Policy & Details', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              AppCard(
                padding: const EdgeInsets.all(16),
                child: Column(
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('Total Refundable Amount', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                        Text('EGP 73', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900, color: scheme.primary)),
                      ],
                    ),
                    const SizedBox(height: 12),
                    const Align(
              ...
              const Text('Upload Supporting Evidence', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
        ...
        // Submit CTA button
        _buildStickyCTAButton('Submit Refund Request', _submitRefundRequest, scheme),
      ],
    );
  }
        ...
        child: const Center(
          child: Column(
            ...
            children: [
              Icon(Icons.add_photo_alternate_outlined, color: Colors.grey, size: 28),
              SizedBox(height: 6),
              Text('Attach Screenshot / Receipt', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
              Text('Select proof of payment/booking error', style: TextStyle(fontSize: 10, color: Colors.grey)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildStickyCTAButton(String label, VoidCallback onPressed, ColorScheme scheme) {
    return Container(
