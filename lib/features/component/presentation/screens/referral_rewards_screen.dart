...
class _ReferralRewardsScreenState extends State<ReferralRewardsScreen>
  ...
  // Mock referral code
  final String _referralCode = 'MEGA-SHUTTLE-77X';

  // Referral states
  int _totalInvites = 12;
  final int _successfulReferrals = 5;
  int _earnedRewardsTotal = 250; // EGP
  int _walletBalance = 100; // EGP available to redeem
  ...
  // List of scratchable vouchers in wallet
  late List<ScratchVoucher> _vouchers;

  // List of contacts
  late List<MockContact> _contacts;

  // Referral History logs
  late List<ReferralHistoryItem> _history;
  ...
  void _initializeMockData() {
    ...
    _history = [
      ReferralHistoryItem(name: 'Sarah Aly', date: 'May 28, 2026', status: 'Completed', rewardAmount: 50),
      ReferralHistoryItem(name: 'Ahmed Kamel', date: 'May 24, 2026', status: 'Completed', rewardAmount: 50),
      ReferralHistoryItem(name: 'Hazem Soliman', date: 'May 20, 2026', status: 'Pending', rewardAmount: 50),
      ReferralHistoryItem(name: 'Mariam Fawzy', date: 'May 18, 2026', status: 'Expired', rewardAmount: 0),
      ReferralHistoryItem(name: 'Tarek Refaat', date: 'May 12, 2026', status: 'Completed', rewardAmount: 50),
      ReferralHistoryItem(name: 'Nour El-Din', date: 'May 08, 2026', status: 'Completed', rewardAmount: 50),
    ];
    ...
    _vouchers = [
      ...
      ScratchVoucher(
        ...
        description: 'Upgrade your standard seat to Premium Luxury standard at no extra cost.',
        promoCode: 'UPGRADEVIP',
        amount: 'VIP SEAT',
        isRevealed: true, // Already scratched
      ),
    ];
  }
  ...
  void _shareReferralLink() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Sharing link via System Share...'),
      ),
    );
  }
  ...
  void _redeemRewards() {
    ...
    setState(() {
      _earnedRewardsTotal += _walletBalance;
      _walletBalance = 0;
    });
            ...
            return Container(
              ...
              child: Column(
                children: [
                  ...
                  const Text('Scratch card to reveal your promotional reward code!', style: TextStyle(fontSize: 11, color: Colors.grey)),
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        Text(
                                          voucher.promoCode,
                                          style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: scheme.primary, letterSpacing: 1),
                                        ),
                              ...
                              GestureDetector(
                                onPanUpdate: (details) {
                                  ...
                                  if (renderBox != null) {
                                    ...
                                    final cardOffset = Offset(localPos.dx - 48, localPos.dy - 100);
                                    setModalState(() {
                                ...
                                child: CustomPaint(
                                  ...
                                  painter: ScratchCardPainter(_scratchPoints, scheme),
                                ),
                              ),
                          ],
                        ),
                  ...
                  AppButton(
                    label: _scratchCompleted ? 'Claim Reward' : 'Scratch card to reveal',
                    onPressed: () {
                      Navigator.of(context).pop();
                      if (!_scratchCompleted) {
                        setState(() {
  ...
  Widget _buildMilestoneProgressCard(ColorScheme scheme) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [scheme.primary.withAlpha(50), scheme.tertiary.withAlpha(20)]),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(fontSize: 11, color: scheme.onSurface.withAlpha(180), height: 1.3),
                ),
  ...
  Widget _buildStatsGrid(ColorScheme scheme) {
    return GridView.count(
      ...
      mainAxisSpacing: 10,
      children: [
        _buildStatCard('Total Invites', '$_totalInvites', Icons.people_outline_rounded, scheme.secondary),
        _buildStatCard('Successful', '$_successfulReferrals', Icons.check_circle_outline_rounded, Colors.green),
        _buildStatCard('Total Earned', 'EGP $_earnedRewardsTotal', Icons.payments_outlined, scheme.primary),
      ],
    );
  }
  ...
  Widget _buildReferralCodeCard(ColorScheme scheme) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Your Referral Code', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.grey)),
          const SizedBox(height: 10),
          Row(
            children: [
              Expanded(
    ...
    return Column(
      children: [
        _buildQuickNavTile(
          ...
          badgeText: _walletBalance > 0 ? 'EGP $_walletBalance Claimable' : null,
          color: Colors.orangeAccent,
                          ...
                          children: [
                            ...
                            if (badgeText != null) ...[
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(color: Colors.redAccent.withAlpha(40), borderRadius: BorderRadius.circular(6)),
                                child: Text(badgeText, style: const TextStyle(fontSize: 8, color: Colors.redAccent, fontWeight: FontWeight.bold)),
                              ),
                            ]
                          ],
                        ),
  ...
  void _showQRDialog() {
    showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)),
          title: const Center(child: Text('Scan to Join BMT', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ...
              Text(
                ...
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 11, color: scheme.onSurface.withAlpha(150), height: 1.3),
              ),
            ],
          ),
          actions: [
  ...
  Widget _buildInviteShareHeader(ColorScheme scheme) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Direct Share Options', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.grey)),
          const SizedBox(height: 12),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: [
              _buildInviteOptionButton(Icons.share_rounded, 'Share Link', _shareReferralLink, scheme),
              _buildInviteOptionButton(Icons.qr_code_2_rounded, 'Show QR', _showQRDialog, scheme),
              _buildInviteOptionButton(Icons.copy_rounded, 'Copy Code', _copyReferralCode, scheme),
            ],
          )
        ],
      ),
  ...
  Widget _buildInviteOptionButton(IconData icon, String label, VoidCallback onTap, ColorScheme scheme) {
  ...
  Widget _buildContactTile(MockContact contact, ColorScheme scheme) {
    return Container(
      ...
      child: Row(
        children: [
          AppAvatar(initials: contact.name.split(' ').map((e) => e[0]).join(), radius: 18),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                const SizedBox(height: 2),
                Text(contact.detail, style: const TextStyle(fontSize: 10, color: Colors.grey)),
              ],
            ),
          ),
          contact.isInvited
              ? Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      SizedBox(width: 4),
                      Text('Invited', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.green)),
                    ],
                  ),
                )
              : ElevatedButton(
                  ...
                  child: const Text('Invite', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.white)),
                ),
        ],
      ),
    );
    ...
    return ListView(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text('Referrals Log', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
            Text('${_history.length} total referrals', style: const TextStyle(fontSize: 11, color: Colors.grey)),
          ],
        ),
        const SizedBox(height: 12),
        ...
        ..._history.map((item) {
          ...
          return Container(
            ...
            child: Row(
              children: [
                AppAvatar(initials: item.name.split(' ').map((e) => e[0]).join(), radius: 18),
                const SizedBox(width: 14),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      const SizedBox(height: 2),
                      Text('Invited: ${item.date}', style: const TextStyle(fontSize: 10, color: Colors.grey)),
                    ],
                  ),
                ),
                Column(
                  ...
                  children: [
                    Text(
                      item.rewardAmount > 0 ? '+ EGP ${item.rewardAmount}' : 'EGP 0',
                      style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w900,
                        color: item.rewardAmount > 0 ? scheme.primary : Colors.grey,
                      ),
                    ),
                    const SizedBox(height: 4),
                    Container(
  ...
  Widget _buildWalletView(ColorScheme scheme) {
    return ListView(
      ...
      children: [
        ...
        // Unscratched Cards
        const Text('Claim Reward Vouchers', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.grey)),
        const SizedBox(height: 12),
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Available to Redeem', style: TextStyle(fontSize: 10, color: Colors.grey)),
                    const SizedBox(height: 4),
                    Text(
                      'EGP $_walletBalance',
                      style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: scheme.primary),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Pending Clearance', style: TextStyle(fontSize: 10, color: Colors.grey)),
                    const SizedBox(height: 4),
                    Text(
                      'EGP 50',
          ...
          Row(
            ...
            children: [
              Text(
                ...
                style: TextStyle(fontSize: 10, color: scheme.onSurface.withAlpha(150)),
              ),
              ElevatedButton(
                ...
                child: const Text('Redeem', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
              ),
            ],
          ),
        ],
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    height: 46,
                    decoration: BoxDecoration(
                      color: voucher.isRevealed ? scheme.primary.withAlpha(20) : Colors.orangeAccent.withAlpha(20),
                      shape: BoxShape.circle,
                    ),
                    alignment: Alignment.center,
                    child: Icon(
                      voucher.isRevealed ? Icons.card_giftcard_rounded : Icons.lock_outline_rounded,
                      color: voucher.isRevealed ? scheme.primary : Colors.orangeAccent,
                      size: 20,
                    ),
                  ),
                  const SizedBox(width: 14),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        Text(
                          voucher.isRevealed ? 'Revealed Code: ${voucher.promoCode}' : 'Locked - Scratch to reveal',
                          style: TextStyle(
                            fontSize: 10,
                            color: voucher.isRevealed ? Colors.green : Colors.grey,
                            fontWeight: voucher.isRevealed ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                      ],
                    ),
...
class QRPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(const Rect.fromLTWH(8, 8, 24, 24), Paint()..color = Colors.white);
    canvas.drawRect(const Rect.fromLTWH(12, 12, 16, 16), Paint()..color = primaryColor);
    ...
    canvas.drawRect(Rect.fromLTWH(size.width - 28, 12, 16, 16), Paint()..color = primaryColor);
    ...
    canvas.drawRect(Rect.fromLTWH(12, size.height - 28, 16, 16), Paint()..color = primaryColor);
